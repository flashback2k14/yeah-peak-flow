...
1. Read the feature spec referenced by the user (including Tech Design and Design Decisions sections)
1. If no `## Design Decisions` section exists in the feature spec, suggest: "Run `/frontend-design` first to define the visual direction before implementation."
1. Check existing shared components: `ls src/app/shared/components/ 2>/dev/null`
1. Check existing services: `ls src/app/core/services/ 2>/dev/null`
1. Check existing feature components: `ls src/app/features/ 2>/dev/null`
1. Check existing routes: `git ls-files src/app/`
